document.addEventListener("DOMContentLoaded", function () {
    ...
    radios.forEach((radio) => {
      radio.addEventListener("change", () => {
        ...
        const siblings = document.querySelectorAll(
          `.option-card input[name="${groupName}"]`,
        );
  ...
  function setupBedroomCount() {
    const buttons = Array.from(
      document.querySelectorAll("#bedroom-count button"),
    );
  ...
  function showStep(stepIndex) {
    ...
    const progress = Array.from(
      document.querySelectorAll(".quote-progress-segment"),
    );
    ...
    if (nextBtn) {
      ...
      const communicsationConsent = document.getElementById(
        "communicsation-consent",
      );
      const communicsationConsentMobile = document.getElementById(
        "communicsation-consent-mobile",
      );
      if (isLast) {
  ...
  function validateCurrentStep() {
    ...
    const homeType = getRadioValue("homeType");
    const bedroom = document
      .querySelector("#bedroom-count button.selected")
    ...
    switch (step) {
      ...
      case 7:
        ...
        if (!consentChecked) {
          showError(
            "Please agree to receive communications from Comfort Movers.",
          );
          return false;
        }
  ...
  async function submitQuote() {
    ...
    const fullPhoneNumber = normalizePhoneForPayload(
      phoneIti ? phoneIti.getNumber() : normalizePhoneInput(phoneInput),
    );
    ...
    try {
      const response = await fetch(
        "https://comfortcare.co.nz/cm/api/create-order",
        {
          method: "POST",
          headers: {
  ...
  if (cvInput) {
    cvInput.addEventListener("change", function () {
      fileLabel.innerText =
        this.files.length > 0 ? this.files[0].name : "Upload your CV";
    });
  }
  ...
  if (careerBtn && !careerBtn.dataset.bound) {
    ...
    careerBtn.addEventListener("click", async function () {
      ...
      if (!validatePhone(phoneInput, careerPhoneIti))
      ...
      const fullPhone = careerPhoneIti
        ? careerPhoneIti.getNumber()
        : normalizePhoneInput(phoneInput);
      ...
      try {
        const response = await fetch(
          "https://comfortcare.co.nz/cm/api/carrers",
          {
            method: "POST",
            body: formData,
            headers: {
